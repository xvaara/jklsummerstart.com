  ...
  <div class="container-fluid">
    <div class="d-flex my-3">
      ...
      <div class="row">
        ...
        <div class="col-12 col-md-8 col-md-offset-4 col-lg-6">
              ...
              <table class="table table-condensed table-striped-columns" style="--bs-body-bg: transparent; --bs-emphasis-color: white;">
                <tbody>
                  ...
                  <tr v-for="row in g.rows" :class="{ tauko: row.team1 === 'TAUKO' }">
                    <td width="20%">{{ truncate(row.weekday,2) }} <strong>{{ row.time }}</strong> <span style="white-space: nowrap;"> {{ row.field }}</span> </td>
                    <td width="30%"> {{ row.team1 }} <span v-if="row.pool1"> ({{ row.pool1 }})</span> </td>
                    <td width="10%" style="white-space: nowrap;"> <span v-if="row.score1"> {{ row.score1 }} - {{ row.score2 }} </span> </td>
                    <td width="30%"> {{ row.team2 }} <span v-if="row.pool2"> ({{ row.pool2 }})</span> </td>
                    <td width="10%" class="sshideextra">
                      <a v-if="row.link" :href="row.link">
                        <span class="badge"> {{ row.tpool }} </span>
                      </a>
                      <span v-else class="badge"> {{ row.tpool }}</span>
                    </td>
                </tr>
                </tbody>
              </table>
